...
## Creating a Release
  ...
  1. Go to the [Releases](https://github.com/elliotgoodrich/trimja/releases)
     page and click **Draft a new release**.
  2. Under **Choose a tag**, type a new tag of the form `vA.B.C` following
     [semantic versioning](https://semver.org/) and select
     **Create new tag on publish**. Leave the target as `main`.
  3. Give the release a title (e.g. `vA.B.C`) and generate the release notes.
